...
import org.sword.wechat4j.pay.protocol.hongbao.HongBaoRequest;
import org.sword.wechat4j.pay.protocol.hongbao.HongBaoResponse;
import org.sword.wechat4j.pay.protocol.hongbao.query.Hbinfo;
import org.sword.wechat4j.pay.protocol.hongbao.query.HongBaoQueryRequest;
import org.sword.wechat4j.pay.protocol.hongbao.query.HongBaoQueryResponse;
...
import org.sword.wechat4j.pay.protocol.transfer.TransferRequest;
import org.sword.wechat4j.pay.protocol.transfer.TransferResponse;
import org.sword.wechat4j.pay.protocol.transfer.query.TransferQueryRequest;
import org.sword.wechat4j.pay.protocol.transfer.query.TransferQueryResponse;
...
public class PayManager {
     ...
     * 转账
     */
    private static final String HTTPS_API_MCH_WEIXIN_QQ_COM_PAY_TRANSFER = "https://api.mch.weixin.qq.com/mmpaymkttransfers/promotion/transfers";
    /**
     * 转账查询
     */
    private static final String HTTPS_API_MCH_WEIXIN_QQ_COM_PAY_TRANSFER_QUERY = "https://api.mch.weixin.qq.com/mmpaymkttransfers/gettransferinfo";
    /**
     * 红包
     */
    private static final String HttPS_API_MCH_WEIXIN_QQ_COM_PAY_HONGBAO = "https://api.mch.weixin.qq.com/mmpaymkttransfers/sendredpack";
    /**
     * 红包查询
     */
    private static final String HTTPS_API_MCH_WEIXIN_QQ_COM_PAY_HONGBAO_QUERY = "https://api.mch.weixin.qq.com/mmpaymkttransfers/gethbinfo";
    /**
     * 发起转账
     ...
     * 
     * @date 2016年2月23日 下午2:33:10
     */
    public static TransferResponse transfer(TransferRequest request) throws PayApiException, PayBusinessException, SignatureException {
        JaxbParser requestParser  = buildJAXBParser(TransferRequest.class);
        JaxbParser responseParser = buildJAXBParser(TransferResponse.class);
        request.setSign(signature(request));
        String postData = requestParser.toXML(request);
        logger.info("post data \n" + postData);
        String postResult = clientCustomSSL(HTTPS_API_MCH_WEIXIN_QQ_COM_PAY_TRANSFER,postData,Config.instance().getMchId(),Config.instance().getCertLocation());
        logger.info("post result \n" + postResult);
        validResponseSign(postData);    	
        checkAccess(postResult);
        checkBusiness(postResult);
        TransferResponse response = (TransferResponse) responseParser.toObj(postResult);
        return response;
    }
    /**
     * 转账查询
     * @param request 转账查询请求对象
     * @return
     * @throws PayApiException 
     * @throws PayBusinessException 
     * @throws SignatureException 
     * 
     * @date 2016年2月23日 下午2:34:32
     */
    public static TransferQueryResponse transferQuery(TransferQueryRequest request) throws PayApiException, PayBusinessException, SignatureException{
    	JaxbParser requestParser  = buildJAXBParser(TransferQueryRequest.class);
    	JaxbParser responseParser = buildJAXBParser(TransferQueryResponse.class);
        request.setSign(signature(request));
        String postData = requestParser.toXML(request);
        logger.info("post data \n" + postData);
        String postResult = clientCustomSSL(HTTPS_API_MCH_WEIXIN_QQ_COM_PAY_TRANSFER_QUERY,postData,Config.instance().getMchId(),Config.instance().getCertLocation());
        logger.info("post result \n" + postResult);
        checkAccess(postResult);
        checkBusiness(postResult);
        validResponseSign(postData);
        TransferQueryResponse response = (TransferQueryResponse) responseParser.toObj(postResult);
        return response;    	
    }
    /**
     * 发送红包
     * @param request 红包请求对象
     * @return
     * @throws PayApiException
     * @throws PayBusinessException
     * @throws SignatureException 
     * 
     * @date 2016年2月23日 下午2:49:15
     */
    public static HongBaoResponse sendHongBao(HongBaoRequest request) throws PayApiException, PayBusinessException, SignatureException{
    	JaxbParser requestParser  = buildJAXBParser(HongBaoRequest.class);
    	JaxbParser responseParser = buildJAXBParser(HongBaoResponse.class);
        request.setSign(signature(request));
        String postData = requestParser.toXML(request);
        logger.info("post data \n" + postData);
        String postResult = clientCustomSSL(HttPS_API_MCH_WEIXIN_QQ_COM_PAY_HONGBAO,postData,Config.instance().getMchId(),Config.instance().getCertLocation());
        logger.info("post result \n" + postResult);
        validResponseSign(postData);
        checkAccess(postResult);
        checkBusiness(postResult);
        HongBaoResponse response = (HongBaoResponse) responseParser.toObj(postResult);
        return response;
    }
    /**
     * 红包查询
     ...
     * @throws PayBusinessException
     * 
     * @date 2016年2月23日 下午8:25:19
     */
    public static  HongBaoQueryResponse hongBaoQuery(HongBaoQueryRequest request) throws SignatureException, PayApiException, PayBusinessException{
    	JaxbParser requestParser  = buildJAXBParser(HongBaoQueryRequest.class);
    	JaxbParser responseParser = buildJAXBParser(HongBaoQueryResponse.class);
        request.setSign(signature(request));
        String postData = requestParser.toXML(request);
        logger.info("post data \n" + postData);
        String postResult = clientCustomSSL(HTTPS_API_MCH_WEIXIN_QQ_COM_PAY_HONGBAO_QUERY,postData,Config.instance().getMchId(),Config.instance().getCertLocation());
        logger.info("post result \n" + postResult);
        validResponseSign(postData);       	
        checkAccess(postResult);
        checkBusiness(postResult);
        HongBaoQueryResponse response = (HongBaoQueryResponse) responseParser.toObj(postResult);
        try {
			Map<String, Object> map = getMapFromXMLString(postResult);
			System.out.println("hbinfo--->"+map.get("hblist"));
			String[] hblist = ((String) map.get("hblist")).split("\n");
//			[0]-->
//			[1]-->
//			[2]-->osU7_v0Sl-gn9sGdftNU8hErZIbY
//			[3]-->RECEIVED
//			[4]-->100
//			[5]-->2016-02-22 11:51:17			
			Hbinfo hb = new Hbinfo();//微信文档是这样命名hbinfo的-.-!!
			hb.setOpenid(hblist[2]);
			hb.setStatus(hblist[3]);
			hb.setAmount(hblist[4]);
			hb.setRcv_time(hblist[5]);
			response.setHblist(hb);////这里只考虑发一个红包的情况，不考虑发群红包
		} catch (Exception e) {
			  logger.error("红包查询失败", e);
			// TODO Auto-generated catch block
			//never throws,cuz after checkAccess()
		}
        System.out.println(response);
        return response;    	
    } 
    public static Object test(Object request,Object response) throws NoSuchMethodException, SecurityException, SignatureException, PayBusinessException, PayApiException{
...
//        validResponseSign(postResult);
	...
	private static Map<String, Object> getMapFromXMLString(String xmlString) throws ParserConfigurationException, IOException, SAXException {
    	//转自带
    	try {
			return XMLUtil.xmlToMap(xmlString, false);
	...
	public static String clientCustomSSL(String url ,String data,String keyPassword,String  certificatePathName){
		try{
	        ...
	        File file = new File(certificatePathName);
	        ...
	        try {
		        HttpPost httpPost = new HttpPost(url);//处理的接口
	...
	private static Object process(Object request,Object response) throws SignatureException, PayBusinessException, PayApiException, NoSuchMethodException, SecurityException{
        JaxbParser requestParser  = buildJAXBParser(TransferRequest.class);
        JaxbParser responseParser = buildJAXBParser(TransferResponse.class);
        Method signMethod = request.getClass().getMethod("setSign",String.class);
        try {
			signMethod.invoke(request, signature(request));
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
        ...
        checkAccess(postResult);
        checkBusiness(postResult);
        return responseParser.toObj(postResult);
	}
}
