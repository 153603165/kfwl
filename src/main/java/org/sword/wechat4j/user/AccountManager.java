...
public class AccountManager {
	...
	public String shortUrl(String longUrl) {
		...
		String resultStr = HttpUtils.post(
				SHORTURL_POST_URL + TokenProxy.accessToken(), requestData);
	...
	public Qrcode createQrcodePerpetual(long sceneId) {
		return createQrcodeTicket(QrcodeType.QR_LIMIT_SCENE, null, sceneId,
				null);
	}
	...
	public Qrcode createQrcodePerpetualstr(String sceneStr) {
		return createQrcodeTicket(QrcodeType.QR_LIMIT_STR_SCENE, null, null,
				sceneStr);
	}
	...
	public Qrcode createQrcodeTemporary(long sceneId, int expireSeconds) {
		return createQrcodeTicket(QrcodeType.QR_SCENE, expireSeconds, sceneId,
				null);
	}

	private Qrcode createQrcodeTicket(QrcodeType qrcodeType,
			Integer expireSeconds, Long sceneId, String sceneStr) {
		...
		logger.info("request data " + requestData);
		String resultStr = HttpUtils.post(
				QRCODE_POST_URL + TokenProxy.accessToken(), requestData);
		logger.info("return data " + resultStr);
		try {
			WeChatUtil.isSuccess(resultStr);
		} catch (WeChatException e) {
			resultStr = HttpUtils.post(
					QRCODE_POST_URL + TokenProxy.accessToken(), requestData);
			try {
				WeChatUtil.isSuccess(resultStr);
			} catch (WeChatException e1) {
				logger.error(e.getMessage());
				e.printStackTrace();
				return null;
			}
