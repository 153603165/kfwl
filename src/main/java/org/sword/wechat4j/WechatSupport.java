...
public abstract class WechatSupport {
	...
	private String response(){
		...
		try {
			...
			result = wechatResponse.getContent()==null?jaxbParser.toXML(wechatResponse):wechatResponse.getContent().equals("noResponse") ? "" : jaxbParser.toXML(wechatResponse);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return result;
	 ...
	 * @param content
	 * 
	 * @date 2016年3月23日 上午10:09:54
	 */
	public void responseText(){
		responseBase();
		wechatResponse.setMsgType(MsgType.text.name());
		wechatResponse.setContent("noResponse");
	}
